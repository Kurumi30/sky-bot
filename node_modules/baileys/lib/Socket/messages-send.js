// Alterado por: Dev Gui
import NodeCache from '@cacheable/node-cache';
import { Boom } from '@hapi/boom';
import { proto } from '../../WAProto/index.js';
import { DEFAULT_CACHE_TTLS, WA_DEFAULT_EPHEMERAL } from '../Defaults/index.js';
import { aggregateMessageKeysNotFromMe, assertMediaContent, assertMeId, bindWaitForEvent, decryptMediaRetryData, DEF_MEDIA_HOST, encodeNewsletterMessage, encodeSignedDeviceIdentity, encodeWAMessage, encryptMediaRetryRequest, extractDeviceJids, generateMessageIDV2, generateParticipantHashV2, generateWAMessage, getContentType, getStatusCodeForMediaRetry, getUrlFromDirectPath, getWAUploadToServer, MessageRetryManager, normalizeMessageContent, parseAndInjectE2ESessions, unixTimestampSeconds } from '../Utils/index.js';
...
export const makeMessagesSocket = (config) => {
    ...
    const createParticipantNodes = async (recipientJids, message, extraAttrs, dsmMessage) => {
        ...
        const encryptionPromises = patchedMessages.map(async ({ recipientJid: jid, message: patchedMessage }) => {
            try {
                if (!jid)
                    return null;
                let msgToEncrypt = patchMessageRequiresBeforeSending(patchedMessage);
                if (dsmMessage) {
                    ...
                    if (isOwnUser && !isExactSenderDevice) {
                        msgToEncrypt = patchMessageRequiresBeforeSending(dsmMessage);
    ...
    /**
     * The native_flow node must announce the kind of button actually being sent,
     * otherwise WhatsApp silently drops non quick_reply buttons (cta_url, single_select, ...)
     */
